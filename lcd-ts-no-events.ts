...
class Lcd {
  ...
  async clear(cb: CB): Promise<void> {
    ...
    await this.commandAndDelay(COMMANDS.CLEAR_DISPLAY, 3, cb);
  }

  async home(cb: CB): Promise<void> {
    // Wait > 1.52ms. There were issues waiting for 2ms so wait 3ms.
    await this.commandAndDelay(COMMANDS.HOME, 3, cb);
  }
  ...
  private commandAndDelay(command: number, timeout: number, cb: CB): Promise<void> {
    return new Promise<void>(resolve => {
      this.lock(async (release) => {
        if (this.tryCommand(command, cb)) {
          await delay(timeout);
          this.callCB(null, null, cb);
        }
        release();
        resolve();
      });
  ...
  private async tryCommand(command: number, cb: CB): Promise<boolean> {
    try {
      ...
      this.callCB(e, undefined, cb);
      return false;
    }
  }

  private callCB(err: Error | null, value: any, cb: CB) {
    if (err) {
      cb(err);
    } else {
      cb(value);
